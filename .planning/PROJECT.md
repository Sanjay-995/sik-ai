...
Sik AI is an iOS app that measures the user's body using only the iPhone's camera and tracks how those measurements change over time. **v1 is positioned for the weight-loss audience** — users who want to see cm-scale changes from diet and cardio over weeks. A future v2 will extend the product to gym / hypertrophy users with mm-scale precision tracking; that audience requires a parametric body-shape model and license work that we are deferring. The closest reference product is ZOZOFIT, but Sik AI is **suitless**: no special clothing or hardware, just the phone.
...
- [ ] **Real on-device body-measurement engine for the weight-loss audience** — replace the simulated scanner with real capture and measurement using the iPhone camera + Apple Vision 3D body pose (`VNHumanBodyPose3DObservation`, iOS 17+) + ARKit world tracking for scale calibration. Circumferences are derived using **heuristic / cylindrical models around 3D pose keypoints**, NOT a parametric body-shape model. Target accuracy: **±2cm test-retest reproducibility** for waist / hip / chest, sufficient for cm-scale weight-loss tracking. Gym-grade precision (±0.5cm for hypertrophy tracking) is **deferred to v2** with a body-model integration.
- [ ] **Spike-first feasibility gate** — before committing to full productization, run a 2-3 week technical spike that measures real bodies against tape-measure ground truth across diverse BMI categories and produces a go/no-go decision based on actual data. Reproducibility (same body, two scans, two days) is the primary metric, not just absolute accuracy.
- [ ] **Capture protocol + UX** — two-shot capture (front + side) with strict pose / framing / lighting / clothing guidance, retake prompts, and quality gates. Compensates for the lower model power by tightening capture conditions. Reuses the existing scan-flow visuals as polish on top of real capture.
- [ ] **Real measurement output for v1** — produce real values for circumferences the heuristic approach can support (waist, hip, chest, neck, shoulders, arms, thighs). Body-fat and muscle-mass *estimates* are surfaced with **explicit "estimate" labeling** since they cannot be reliably derived from heuristic measurements alone — full body-fat / muscle-mass measurement is deferred to v2 with the body model. Weight + BMI are user-input. Score derivation rules to be defined.
...
- **Licensed third-party body-measurement SDKs** (3DLOOK, Bodygee, Sizer, etc.) — rejected for v1 because pre-revenue economics make per-scan or annual license fees a poor fit. Revisit only if the spike shows DIY can't hit the weight-loss accuracy bar.
...
- **Parametric body-shape models (SMPL, SMPL-X, SHAPY, PIXIE, 4D-Humans, STAR) for v1** — rejected. SMPL-family weights are non-commercial-licensed by MPI-IS, and a commercial license requires paid negotiation with unknown cost and weeks-long timelines. Without a body-shape model we sacrifice gym-grade reproducibility, which is acceptable because v1 targets the weight-loss audience where ±2cm is enough. Body-shape model integration is deferred to v2 along with the gym positioning.
- **Gym / hypertrophy-grade precision tracking for v1** — rejected. Tracking mm-scale weekly muscle-circumference changes requires the body-model path that we are deferring. The product positioning, marketing copy, and onboarding for v1 must NOT promise this capability.
...
- For the weight-loss audience, ±2cm is acceptable because cm-scale changes happen quickly. This is the v1 target.
- Open-source parametric body models (SMPL, SMPL-X, SHAPY) are technically mature for on-device CoreML inference but **all carry non-commercial licenses**. Commercial use requires paid negotiation with MPI-IS (cost unknown). For v1 we sidestep this entirely by not using a body-shape model; v2 will revisit the license question with revenue and real users.
...
- **Accuracy bar (v1)**: ±2cm test-retest reproducibility on waist / hip / chest circumferences across diverse BMI categories. This is intentionally weight-loss-grade, not gym-grade. ±0.5cm reproducibility is a v2 goal contingent on body-model integration.
...
| **v1 ships WITHOUT a parametric body-shape model** (no SMPL / SMPL-X / SHAPY) | Removes the MPI commercial-license risk entirely (unknown cost, weeks-long negotiation). Sacrifices gym-grade reproducibility, which is acceptable because v1 targets weight-loss users where ±2cm suffices. Body-model integration is a v2 milestone with the gym positioning. | — Pending |
| **v1 audience: weight-loss users only** | Cm-scale changes are detectable with heuristic measurement; mm-scale gym tracking is not. Marketing must reflect this. Gym-user product is v2. | — Pending |
| Measurement primitive: Apple Vision 3D body pose + ARKit scale + cylindrical heuristics | Only commercially-clean stack available for on-device measurement without a body-shape model. Free, no license risk, on every iOS 17+ iPhone. | — Pending |
| Spike before product build | A 2-3 week feasibility test reduces the risk of investing months and missing the accuracy bar. Test-retest reproducibility is the primary metric, not absolute accuracy. Spike must include diverse BMI testers. | — Pending |
| Two-shot capture (front + side), not single-shot or 360° | Single-shot has irresolvable depth ambiguity; 360° video has painful UX. Two-shot is the published industry standard for accuracy/UX balance. | — Pending |
| Camera library: react-native-vision-camera (MIT) | The default `expo-camera` does not expose pixel-level frame buffers; ML inference is impossible with it. VisionCamera is the standard 2026 path. Note: requires `expo prebuild` (bare workflow + EAS Build). | — Pending |
| Native bridge: Expo Modules API + Turbo Native Modules (New Architecture) | Existing project uses New Architecture. Expo Modules generates type-safe Swift↔TypeScript bindings cleanly. | — Pending |
| Image processing happens entirely in native Swift, never crosses the RN bridge | Sending raw camera buffers across the bridge is the most common iOS-RN ML integration failure (YUV format mismatches, memory pressure). Only final scalar measurements cross the bridge. | — Pending |
...
| Body imagery is processed ephemerally and discarded | Raw frames must NOT be persisted to AsyncStorage (which gets backed up to iCloud by default). Only final measurements are persisted. | — Pending |
...
---
*Last updated: 2026-04-30 after research-driven scope refinement (deferred body-model & gym positioning to v2)*
