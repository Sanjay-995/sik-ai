...
import {
  ...
  Switch, Platform, TextInput, Alert, Share
} from 'react-native';
...
function SettingRow({ icon, label, value, onPress, toggle, toggleValue, onToggle, destructive, accent }: SettingRowProps) {
      ...
      <View style={styles.settingLeft}>
        <View style={[styles.settingIcon, {
          ...
          <Feather
            name={icon as any}
            size={16}
            color={destructive ? colors.destructive : accent ? colors.emerald : colors.textSecondary}
          />
        </View>
...
export default function SettingsScreen() {
  ...
  const { profile, updateProfile, scanHistory } = useApp();
  const { fmtHt, fmtWt } = useUnits();
  ...
  function handleUnitsToggle() {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    const current = profile.units ?? 'metric';
    Alert.alert(
      'Unit System',
      'Choose your preferred measurement units. All measurements will be converted.',
      [
        {
          text: 'Metric (cm, kg)',
          onPress: () => updateProfile({ units: 'metric' }),
          style: current === 'metric' ? 'default' : 'default',
        },
        {
  ...
  async function handleExportCSV() {
    if (scanHistory.length === 0) {
      Alert.alert('No Data', 'Complete at least one scan before exporting.');
      return;
    }
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    const header = 'Date,Score,Weight (kg),BMI,Body Fat (%),Muscle Mass (kg),Chest (cm),Waist (cm),Hips (cm),L.Arm (cm),R.Arm (cm),L.Thigh (cm),R.Thigh (cm),Neck (cm),Shoulders (cm)';
    const rows = scanHistory.map(s => [
      new Date(s.date).toLocaleDateString(),
      s.score,
      s.weight.toFixed(1),
      s.bmi,
      s.measurements.bodyFat.toFixed(1),
      s.measurements.muscleMass.toFixed(1),
      s.measurements.chest.toFixed(1),
      s.measurements.waist.toFixed(1),
      s.measurements.hips.toFixed(1),
      s.measurements.leftArm.toFixed(1),
      s.measurements.rightArm.toFixed(1),
      s.measurements.leftThigh.toFixed(1),
      s.measurements.rightThigh.toFixed(1),
      s.measurements.neck.toFixed(1),
      s.measurements.shoulders.toFixed(1),
    ].join(','));
    const csv = [header, ...rows].join('\n');
    try {
      await Share.share({
        message: csv,
        title: 'Sik AI — Scan History',
      });
    } catch {
  ...
  const currentUnitsLabel = (profile.units ?? 'metric') === 'imperial' ? 'Imperial (in, lbs)' : 'Metric (cm, kg)';
        ...
        {isEditingName ? (
          ...
          <View style={styles.profileInfo}>
            ...
            <Text style={[styles.profileDetail, { color: colors.textSecondary }]}>
              {profile.age}y · {fmtHt(profile.height)} · {fmtWt(profile.weight)}
            </Text>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[styles.upgradeSubtitle, { color: colors.textSecondary }]}>
              7-day free trial · Unlimited scans · AI coaching
            </Text>
          </View>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.settingsGroup, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <SettingRow
            ...
            value={`${scanHistory.length} scans`}
